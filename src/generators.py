...
def number_generator(beginning: int, finish: int) -> Iterator:
    for x in range(beginning, finish + 1):
        yield "0" * (16 - len(str(x))) + str(x)


def card_number_generator(beginning: int, finish: int):
    numbers = number_generator(beginning, finish)
    for i in range(finish - beginning + 1):
        str_ = str(next(numbers))
